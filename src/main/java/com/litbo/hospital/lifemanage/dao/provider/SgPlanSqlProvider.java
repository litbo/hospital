...
//    @Autowired
    public String selectPlan(@Param("planName") String planName, @Param("planDate") Date planDate, @Param("userId") List<String> userId) {
        SQL sql = new SQL();
        sql.SELECT("sg_plan.plan_check,sg_plan.plan_file,sg_plan.panying_sum,sg_plan.pankui_sum,sg_plan.yipan_sum,sg_plan.plan_sum,sg_plan.id,sg_plan.bm_id as bmId, s_emp.user_xm as userId, s_bm.bm_name as bmName, sg_plan.plan_name,sg_plan.plan_date");
        sql.FROM("sg_plan");
