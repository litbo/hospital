...
public class DictProvider {
    ...
    public String listDictByTName(String tName) {
        ...
        if("s_jg".equals(tName))sql.append("SELECT jg_id as dictId ,jg as dictName ,b_name FROM s_jg");
        if("s_mz".equals(tName))sql.append("SELECT mz_id as dictId , mz as dictName ,b_name FROM s_mz");
        if("s_xllb".equals(tName))sql.append("SELECT xllb_id as dictId , xllb as dictName ,b_name FROM s_xllb");
        if("s_sex".equals(tName))sql.append("SELECT sex_id as dictId , sex as dictName ,b_name FROM s_sex");
        if("s_gb".equals(tName))sql.append("SELECT gb_id as dictId , gb as dictName ,b_name FROM s_gb");
        if("s_zwlb".equals(tName))sql.append("SELECT zwlb_id as dictId , zwlb as dictName ,b_name FROM s_zwlb");
        if("s_zzmm".equals(tName))sql.append("SELECT zzmm_id as dictId , zzmm as dictName ,b_name FROM s_zzmm");
        if("eq_cslb".equals(tName))sql.append("SELECT eq_cslb_id as dictId ,eq_cslb_name as dictName ,b_name FROM eq_cslb");
        if("eq_cxfl".equals(tName))sql.append("SELECT eq_cxfl_id as dictId ,eq_cxfl_name as dictName ,b_name FROM eq_cxfl");
        if("eq_gzlb".equals(tName))sql.append("SELECT gzlb_id as dictId ,gzlb_name as dictName ,b_name FROM eq_gzlb");
        if("eq_jldw".equals(tName))sql.append("SELECT eq_jldw_id as dictId ,eq_jldw_name as dictName ,b_name FROM eq_jldw");
        if("eq_qdfs".equals(tName))sql.append("SELECT qdfs_id as dictId ,qdfs_name as dictName ,b_name FROM eq_qdfs");
        if("eq_zjly".equals(tName))sql.append("SELECT zjly_id as dictId ,zjly_name as dictName ,b_name FROM eq_zjly");
        if("eq_syxz".equals(tName))sql.append("SELECT syxz_id as dictId ,syxz_name as dictName ,b_name FROM eq_syxz");
        return sql.toString();
    }
    ...
    public String delByTNameAndId(String tName,String id) {
        StringBuffer sql =new StringBuffer();
        if("s_jg".equals(tName))sql.append("Delete FROM s_jg where jg_id = #{id}");
        if("s_mz".equals(tName))sql.append("Delete FROM s_mz where mz_id = #{id} ");
        if("s_xllb".equals(tName))sql.append("Delete FROM s_xllb where xllb_id = #{id}  ");
        if("s_sex".equals(tName))sql.append("Delete FROM s_sex where sex_id = #{id}  ");
        if("s_gb".equals(tName))sql.append("Delete FROM s_gb where gb_id = #{id}  ");
        if("s_zwlb".equals(tName))sql.append("Delete FROM s_zwlb where zwlb_id = #{id}  ");
        if("s_zzmm".equals(tName))sql.append("Delete FROM s_zzmm where zzmm_id = #{id}  ");
        if("eq_cslb".equals(tName))sql.append("Delete FROM eq_cslb where eq_cslb_id = #{id}  ");
        if("eq_cxfl".equals(tName))sql.append("Delete FROM eq_cxfl where eq_cxfl_id = #{id}  ");
        if("eq_gzlb".equals(tName))sql.append("Delete FROM eq_gzlb where gzlb_id = #{id}  ");
        if("eq_jldw".equals(tName))sql.append("Delete FROM eq_jldw where eq_jldw_id = #{id} ");
        if("eq_qdfs".equals(tName))sql.append("Delete FROM eq_qdfs where qdfs_id = #{id} ");
        if("eq_zjly".equals(tName))sql.append("Delete FROM eq_zjly where zjly_id = #{id} ");
        if("eq_syxz".equals(tName))sql.append("Delete FROM eq_syxz where syxz_id = #{id} ");
        return sql.toString();
    }
}
