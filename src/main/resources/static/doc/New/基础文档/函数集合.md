...
- `confirm` 参数：是否需要弹出确认框
- `id` 参数：需要操作的表格ID  **必填**
- `filter` 参数：需要操作的表格DOM的lay-filter  **必填**
- `name` 参数：需要发送给后台的数据的名称  **必填**
- `reload` 参数：数据提交成功后是否刷新页面
- `event` 参数：需要匹配的按钮的lay-event  **必填**
- `add` 参数：除了所要发送的选中数据外，要发送的其他数据
- `reTable` 参数：重新加载表格,使用欧泰返回数据（默认不重新加载表格）
  - true => 重新加载表格
  - false => 不重新加载表格
